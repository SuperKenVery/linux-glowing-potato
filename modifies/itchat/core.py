...
class session(requests.Session):
    def __init__(self,errHandle,retry=10):
        self.err=errHandle if isinstance(errHandle,list) else [errHandle]
        self.retry=retry
        requests.Session.__init__(self)
    def get(self,*argv,**argvs):
        left=self.retry
        while left>0:
            try:
                return requests.Session.get(self,*argv,**argvs)
...
class Core(object):
    def __init__(self):
        ...
        self.s = session(self.functionDict['Error'])
